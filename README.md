...
This project implements a Zero Downtime deployment strategy.

### The Concept
* **Blue**: The stable version currently serving production traffic.
* **Green**: The new candidate version being deployed.
* **Reverse Proxy**: The router directing users to the active version.

[ Client ] ---> [ Nginx Proxy ] --(active: blue)--> [ App Blue v1 ]
                                  |
                                  --(inactive)----> [ App Green v2 ]

### Deployment Flow
The CI pipeline (`ci.yaml`) orchestrates the deployment via the `scripts/deploy.ps1` script:

1.  **Detection**: Identifies the currently active color (e.g., Blue).
2.  **Startup**: Launches the inactive stack (Green) with the new Docker images.
3.  **Healthcheck**: The proxy attempts to reach the new Green stack internally.
    * ❌ **Failure**: The deployment aborts, Green is stopped, and Blue remains active (No downtime).
    * ✅ **Success**: Proceed to switch.
4.  **Switch**: The Nginx configuration file is updated to point to Green.
5.  **Reload**: Nginx reloads the configuration instantly.
6.  **Backup**: The Blue stack remains running to allow for an immediate rollback if a functional issue is found later.

### Activation Conditions
The `deploy` stage is only executed if the commit is pushed to the **`main`** branch.